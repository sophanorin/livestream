...
export default class Spotlights {
    ...
    addPeerToPrioritySpotlight(peerId) {
    ...
    removePeerFromSelectedSpotlights(peerId) {
        ...
        this._selectedSpotlights = this._selectedSpotlights.filter(
            (peer) => peer !== peerId
        );
    ...
    newPeer(id) {
        logger.debug('room "newpeer" event [id: "%s"]', id);

        if (this._peerList.indexOf(id) === -1) {
            // We don't have this peer in the list
    ...
    closePeer(id) {
        logger.debug('room "peerClosed" event [peerId:%o]', id);

        this._peerList = this._peerList.filter((peer) => peer !== id);
        this._activeVideoConsumers = this._activeVideoConsumers.filter(
            (consumer) => consumer.peerId !== id
        );
        this._selectedSpotlights = this._selectedSpotlights.filter(
            (peer) => peer !== id
        );
    ...
    addVideoConsumer(newConsumer) {
        if (
            newConsumer.kind === "video" &&
      (newConsumer.source === "webcam" ||
        newConsumer.source === "extravideo" ||
        newConsumer.source === "screen") &&
      this._activeVideoConsumers.findIndex(
          (consumer) => consumer.consumerId === newConsumer.id
      ) === -1
        ) {
    ...
    resumeVideoConsumer(consumerId) {
        const videoConsumer = this._activeVideoConsumers.find(
            (consumer) => consumer.consumerId === consumerId
        );
    ...
    _hasActiveVideo(peerId) {
        if (
            this._activeVideoConsumers.findIndex(
                (consumer) => consumer.peerId === peerId && !consumer.remotelyPaused
            ) !== -1
        ) {
            return true;
        }
    ...
    _spotlightsUpdated() {
        ...
        if (
            !this._arraysEqual(
                this._currentSpotlights,
                spotlights.slice(0, this._maxSpotlights)
            )
        ) {
    ...
    set maxSpotlights(maxSpotlights) {
        ...
        if (oldMaxSpotlights !== this._maxSpotlights) {this._spotlightsUpdated();}
    }
}
