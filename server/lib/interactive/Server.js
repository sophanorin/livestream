...
const SOCKET_PATH_UNIX = "/tmp/livestream-server.sock";
const SOCKET_PATH_WIN = path.join("\\\\?\\pipe", process.cwd(), "livestream-server");
const SOCKET_PATH =
...
class Interactive {
            ...
            cmd.question("cmd> ", async (input) => {
                ...
                switch (command) {
                    ...
                    case "logLevel": {
                        ...
                        for (const worker of workers.values()) {
                            promises.push(
                                worker.updateSettings({ logLevel: level })
                            );
                        }
                    ...
                    case "logTags": {
                        ...
                        for (const worker of workers.values()) {
                            promises.push(
                                worker.updateSettings({ logTags: tags })
                            );
                        }
                    ...
                    case "dumpRouter": {
                        const id =
                            params[0] || Array.from(routers.keys()).pop();
                    ...
                    case "dumpTransport": {
                        const id =
                            params[0] || Array.from(transports.keys()).pop();
                    ...
                    case "dumpProducer": {
                        const id =
                            params[0] || Array.from(producers.keys()).pop();
                    ...
                    case "dumpConsumer": {
                        const id =
                            params[0] || Array.from(consumers.keys()).pop();
                    ...
                    case "dumpDataProducer": {
                        const id =
                            params[0] || Array.from(dataProducers.keys()).pop();
                    ...
                    case "dumpDataConsumer": {
                        const id =
                            params[0] || Array.from(dataConsumers.keys()).pop();
                    ...
                    case "roomData": {
                        const room = params[0]
                            ? global.rooms.get(params[0])
                            : Array.from(global.rooms.values()).pop();

                        if (!room) this.log("Room not found");

                        if (typeof room.getRoomData === "function")
                            this.log(
                                `RoomData:\n${JSON.stringify(
                                    room.getRoomData(),
                                    null,
    ...
    error(msg) {
        try {
            this._socket.write(
                `${colors.red.bold("ERROR: ")}${colors.red(msg)}\n`
            );
        } catch (error) {}
    }
}
