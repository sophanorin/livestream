...
module.exports = async function (rooms, peers, config) {
  const collect = async function (registry) {
    ...
    const mRooms = new prom.Gauge({
      name: "livestreams_rooms",
      help: "#rooms",
      registers: [registry],
    });

    mRooms.set(rooms.size);
    const mPeers = new prom.Gauge({
      name: "livestreams_peers",
      help: "#peers",
      labelNames: ["room_id"],
      registers: [registry],
    });
  ...
  try {
    ...
    mediasoup.observer.on("newworker", (worker) => {
      ...
      worker.observer.on("close", () => {
        logger.debug(
          `observing close worker ${worker.pid} #${workers.size - 1}`
        );
