...
class Lobby extends EventEmitter {
    ...
    promoteAllPeers() {
        ...
        for (const peer in this._peers) {
            if (!this._peers[peer].closed) this.promotePeer(peer);
        }
    }
    ...
    holdPeer(peer, joinHandler) {
        logger.info('holdPeer() [peer:"%s"]', peer.id);
        ...
        this._notification(peer.socket, "holding", {
            displayName: peer.displayName,
            id: peer.id,
        });
    }
